...
bool pointInit[4]={0};    //recod of factor been used default to be false
vector<curveOrSurface> geoObjs;   //global surface
int factor=5;   //didvide factor
class resultPoints{
	public:
		vector<PointNormal> pn;
		vector<bool> trimpoints;
		int trimcurve;    // position of trim curve in result points vector, default set to -1
		int type;   //type of obj 0--curve 1--trimcurve 2--surface 3--trimedsurface
};
//wrap the resultPoints calss to a vector
class finalPoints{
	public:
		vector<resultPoints> fp;
};
vector<PointNormal> tempTrim;
bool trim=false;
/*result point vector of factor2 to 5*/
finalPoints finalData[4];
//
...
//return the resultPoints object of each curveOrSurface object
resultPoints getResultPoints(curveOrSurface cOS, int currentPosition)
{
	resultPoints rp;
	if(cOS.geoType==0)
	{
		rp.pn=getCurvePoints(factor,cOS);
		rp.type=0;
		rp.trimcurve=-1;
	}
	else 
		if(cOS.geoType==1)
		{
			rp.pn=getSurfacePoints(factor,cOS);
			if(!trim)
			{
				rp.type=2;
				rp.trimcurve=-1;
			...
			{
				rp.type=3;
				rp.trimpoints=getTrimSurface(factor,tempTrim,cOS);
				trim=false;
			}
		}
		else 
...
{
	finalPoints fp;
	vector<resultPoints> rps;
	resultPoints temp;
	for(unsigned int i=0; i<geoObjs.size();++i)
	{
		temp=getResultPoints(geoObjs[i],i);
		rps.push_back(temp);
	}
	fp.fp=rps;
	return fp;
...
//update Final points and set pointInit to true
void updateFinalPoints()
{
	if(pointInit[factor-2]!=true)
	{
		finalData[factor-2]=getFinalPoints();
		pointInit[factor-2]=true;
	}
}
...
//this function display the curve
void displayCurve(vector<PointNormal> curvePoints)
{
	//vector<Vector3D> curvePoints=getCurvePoints(factor, curve);
	for(unsigned int i=0;i<curvePoints.size()-1;++i)
	{
		Vector3D p=curvePoints[i].point;
		Vector3D pp=curvePoints[i+1].point;
		glBegin(GL_LINES);
...
void displaySurface(vector<PointNormal> pn)
{
	int points=2*pow((double)3,(factor-1))+2;
	for(int i=0;i<points-1;++i)
	{
		for(int j=0;j<points-1;++j)
		{
			Vector3D p1=pn[i*points+j].point;
			Vector3D n1=pn[i*points+j].normal;
			Vector3D p2=pn[i*points+j+1].point;
			Vector3D n2=pn[i*points+j+1].normal;
			Vector3D p3=pn[(i+1)*points+j+1].point;
			Vector3D n3=pn[(i+1)*points+j+1].normal;
			Vector3D p4=pn[(i+1)*points+j].point;
			Vector3D n4=pn[(i+1)*points+j].normal;
...
void displayTrimSurface(resultPoints rp)
{
	vector<PointNormal> pn=rp.pn;
	vector<bool> tp=rp.trimpoints;
	int points=2*pow((double)3,(factor-1))+2;
	for(int i=0;i<points-1;++i)
	{
		for(int j=0;j<points-1;++j)
		{
			//if no point in the trim curve
			bool b1=tp[i*points+j];
			bool b2=tp[i*points+j+1];
			bool b3=tp[(i+1)*points+j+1];
			bool b4=tp[(i+1)*points+j];
			Vector3D p1=pn[i*points+j].point;
			Vector3D n1=pn[i*points+j].normal;
			Vector3D p2=pn[i*points+j+1].point;
			Vector3D n2=pn[i*points+j+1].normal;
...
//this function display all objects in the vector
void displayGeoObjs()
{
	GLfloat grey_diffuse[] = {0.6, 0.6, 0.6, 1.0};
	glMaterialfv(GL_FRONT, GL_DIFFUSE, grey_diffuse);
	updateFinalPoints();
	finalPoints f=finalData[factor-2];
	for(unsigned int i=0; i<f.fp.size();++i)
	{
		switch(f.fp[i].type)
		{
			case 0:
				displayCurve(f.fp[i].pn);
				break;
			case 2:
				displaySurface(f.fp[i].pn);
				break;
			case 3:
				displayTrimSurface(f.fp[i]);
				break;
		}
	}
}
