#include"utility.h"
float getRadius(const float &a, const float &b)
{
	return sqrt(a*a+b*b);
}
...
void constructSwept(vector< curveOrSurface> &geoObjs)
{
	...
	curveOrSurface section, transition,surface;
	int size=geoObjs.size();
	WVector3D tempWP;
	Vector3D tempP;
	vector<WVector3D> tempCP;
	section=geoObjs[size-2];   //u
	transition=geoObjs[size-1];  //v
	for(unsigned int i=0; i<transition.controlPoints.size();++i)
		for(unsigned int j=0; j<section.controlPoints.size();++j)
		{
			tempP=transition.controlPoints[i].p+section.controlPoints[j].p;
			tempP=tempP-section.controlPoints[0].p;   //translation to origional position of transition curve
			tempWP.w=transition.controlPoints[i].w*section.controlPoints[j].w;
			tempWP.p=tempP;
			tempCP.push_back(tempWP);
		}
	surface.geoType=1;  //surface
	surface.degreeU=section.degreeU;
	surface.degreeV=transition.degreeU;
	surface.knotsU=section.knotsU;
	surface.knotsV=transition.knotsU;
	surface.controlPoints=tempCP;
	geoObjs.pop_back();  //pop section and transition out of geoObjs
	geoObjs.pop_back();
	geoObjs.push_back(surface);  //push surface into geoObjs
}

void constructRevolved(vector <curveOrSurface> &geoObjs)
{
	curveOrSurface section, surface;
	int size=geoObjs.size();
	section=geoObjs[size-1];
	surface.geoType=1; 
	surface.degreeU=2;
	surface.degreeV=section.degreeU;
	surface.knotsV=section.knotsU;
	surface.knotsU.push_back(0);
	surface.knotsU.push_back(0);
	surface.knotsU.push_back(0.25);
	surface.knotsU.push_back(0.25);
	surface.knotsU.push_back(0.5);
	surface.knotsU.push_back(0.5);
	surface.knotsU.push_back(0.75);
	surface.knotsU.push_back(0.75);
	surface.knotsU.push_back(1);
	surface.knotsU.push_back(1);
	Vector3D origionalP, tempP;
	float radius;
	vector<WVector3D> tempCP;   
	WVector3D tempWV;
	for(unsigned int i=0; i<section.controlPoints.size();++i)
	{
		origionalP=section.controlPoints[i].p;
		radius=getRadius(origionalP.x, origionalP.z);
		float y=origionalP.y;
		float weight=section.controlPoints[i].w;
		//1
		tempP=Vector3D(radius, y, 0);
		tempWV.p=tempP;
		tempWV.w=weight;
		tempCP.push_back(tempWV);
		//2
		tempP=Vector3D(radius, y, 0-radius);
		tempWV.p=tempP;
		tempWV.w=0.7071*weight;
		tempCP.push_back(tempWV);
		//3
		tempP=Vector3D(0, y, 0-radius);
		tempWV.p=tempP;
		tempWV.w=weight;
		tempCP.push_back(tempWV);
		//4
		tempP=Vector3D(0-radius, y, 0-radius);
		tempWV.p=tempP;
		tempWV.w=0.7071*weight;
		tempCP.push_back(tempWV);
		//5
		tempP=Vector3D(0-radius, y, 0);
		tempWV.p=tempP;
		tempWV.w=weight;
		tempCP.push_back(tempWV);
		//6
		tempP=Vector3D(0-radius, y, radius);
		tempWV.p=tempP;
		tempWV.w=0.7071*weight;
		tempCP.push_back(tempWV);
		//7
		tempP=Vector3D(0, y, radius);
		tempWV.p=tempP;
		tempWV.w=weight;
		tempCP.push_back(tempWV);
		//8
		tempP=Vector3D(radius, y, radius);
		tempWV.p=tempP;
		tempWV.w=0.7071*weight;
		tempCP.push_back(tempWV);
		//9
	...
	surface.controlPoints=tempCP;
	geoObjs.pop_back();  //pop out the origional curve
	geoObjs.push_back(surface);  //push the new surface into geoObjs
